...
def get_faiss_engine():
    """Get or create the FAISS similarity engine instance with fallback to offline mode"""
    global faiss_similarity_engine
    if faiss_similarity_engine is None:
        try:
            faiss_similarity_engine = FAISSSimilarityEngine()
        except Exception as e:
            logger.warning(f"Failed to initialize main FAISS engine: {str(e)}")
            logger.info("Falling back to offline FAISS engine...")
            try:
                from .offline_faiss_engine import get_offline_faiss_engine
                faiss_similarity_engine = get_offline_faiss_engine()
            except Exception as fallback_error:
                logger.error(f"Failed to initialize offline FAISS engine: {str(fallback_error)}")
                # Return a dummy engine that handles errors gracefully
                faiss_similarity_engine = DummyFAISSEngine()
    return faiss_similarity_engine

class DummyFAISSEngine:
    """Dummy FAISS engine that handles errors gracefully when no engine is available"""
