#!/usr/bin/env python3
"""
AA Comprehensive Generator - City and Zip Code Integration
...
# Import the extracted city-zip data
try:
    from extracted_city_zip_data_aa_generator import CITY_ZIP_DATA
    print(f"✅ Loaded {len(CITY_ZIP_DATA)} city-zip combinations")
except ImportError:
    print("❌ Could not import CITY_ZIP_DATA. Please run extract_city_zip_from_json.py first")
    CITY_ZIP_DATA = []
...
class AACityZipGenerator:
    ...
    def __init__(self):
        self.city_zip_data = CITY_ZIP_DATA
        self.total_records = len(self.city_zip_data)
        
        if self.total_records == 0:
            print("⚠️ No city-zip data available")
        else:
            print(f"🏙️ Ready to generate AA data with {self.total_records} city-zip combinations")
...
def main():
    ...
    print(f"\n📊 Data Statistics:")
    print(f"   Total city-zip combinations: {generator.total_records}")
